 ...
 */
    ...
    */
    ...
    public function getFraisVisiteurParMois($idVisiteur,$mois){
        $req = "SELECT ff.libelle, li.quantite*ff.montant as somme from visiteur v INNER JOIN fichefrais fi on fi.idVisiteur = v.id INNER JOIN lignefraisforfait li on li.idVisiteur = v.id and li.mois = fi.mois INNER JOIN fraisforfait ff on ff.id = li.idFraisForfait WHERE v.id = '$idVisiteur' AND fi.mois = '$mois';";
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetchAll();
        return $laLigne;
    }

    public function getFraisVisiteurParId($idVisiteur){
        $req = "SELECT libelle, sum(quantite)*fraisforfait.montant as somme FROM lignefraisforfait INNER JOIN fraisforfait ON idFraisforfait = id WHERE idVisiteur = '$idVisiteur' GROUP BY idFraisForfait;";
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetchAll();
        return $laLigne;
    }

    public function ajouterFraisParId($idVisiteur, $date, $idFraisForfait, $qte){
        $req = "INSERT INTO lignefraisforfait ('idVisiteur','mois','idFraisForfait','quantite') VALUES ('$idVisiteur,$date , $idFraisForfait, $qte') ON DUPLICATE KEY UPDATE quantite = VALUES($qte)";
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetchAll();
        return $laLigne;
    }

    public function ajouterFraisParId2($idFraisForfait,$date){
        echo $req="INSERT INTO fichefrais(idVisiteur, mois) VALUES ('$idFraisForfait','$date')
        ON DUPLICATE KEY UPDATE idVisiteur = VALUES($idFraisForfait), mois VALUES ($date)";
        $res = PdoGsb::$monPdo->query($req);
        $laLigne = $res->fetchAll();
        return $laLigne;
    }
